...
item_emb = dict()
user_user_map_matrix = np.random.rand(DIM,DIM)
...
out_uu_matrix = "./data/dataset/ciao/output/matrix_uu"
out_uv_matrix = "./data/dataset/ciao/output/matrix_uv"
...
social_prefer_p = len(user_user_view1) / len(user_user_view2)
print(social_prefer_p)
...
user_item_p = len(user_friend_edges) / len(user_item_edges)
print(user_item_p)
...
def init_all_vec():
    ...
    pk.dump(user_user_map_matrix, open(out_uu_matrix + "_r" + str(reg) + "N" + str(NEG_N) + "_init.pkl", 'wb'))
    ...
    if type == 1:
        if source in all_user_set and target in all_user_set:
            ...
            uu_error = implicit_lr * (1 - sigmoid(user_emb.get(source).dot(user_user_map_matrix).dot(user_emb.get(target)))) * user_user_map_matrix.dot(user_emb.get(
                target)) * lamda1
            # reg part
            reg_part_error = implicit_lr * reg * 2*lamda1 * (user_social_emb.get(source) - user_emb.get(source))
        else:
            uu_error = 0
            reg_part_error = 0
        # u^s_i
...
def update_user_item_vertex(source, target, error, label,type):
    source_emb = user_emb.get(source)
    # 如果是friend推荐
    if type == 1:
        target_emb = user_emb.get(target)
        score = sigmoid(math.pow(-1, label) * source_emb.dot(user_user_map_matrix).dot(target_emb))
        g = math.pow(-1, label+1) * score * explicit_lr
        # total error for source vertex
        error += g * user_user_map_matrix.dot(target_emb)
        if target in user_social_emb and target in user_prefer_emb:
            t_v1 = user_weight_dict[target]["view1"]
            t_v2 = user_weight_dict[target]["view2"]
            t_w1 = t_v1+g*source_emb.dot(user_user_map_matrix).dot(user_social_emb[target]-user_prefer_emb[target])*(t_v1**2)/(t_v1+t_v2)**2
            t_w2 = t_v2+g * source_emb.dot(user_user_map_matrix).dot (
                        user_prefer_emb[target] - user_social_emb[target])* (t_v2 ** 2)  / (t_v1 + t_v2) ** 2
            # new_vec = target_emb + g * source_emb.dot(user_user_map_matrix)
            # user_emb[target] = new_vec
            user_weight_dict[target]["view1"] = get_user_weight(t_w1)
            user_weight_dict[target]["view2"] = get_user_weight(t_w2)
            user_emb[target] = (user_weight_dict[target]["view1"]*user_social_emb[target]+user_weight_dict[target]["view2"]*
                                user_prefer_emb[target])/(user_weight_dict[target]["view1"]+user_weight_dict[target]["view2"])
        else:
            # total error for source vertex
            error += g * user_user_map_matrix.dot(target_emb)
            new_vec = target_emb + g * source_emb.dot(user_user_map_matrix)
    ...
    # 如果是item推荐
        ...
        score = sigmoid(math.pow(-1, label) * source_emb.dot(user_item_map_matrix).dot(target_emb))
        g = math.pow(-1, label) * score * explicit_lr
        # total error for source vertex
        error += g * user_item_map_matrix.dot(target_emb)
        new_vec = target_emb + g * source_emb.dot(user_item_map_matrix)
        item_emb[target] = new_vec


def update_user_item(source, target, neg_vertices,type):
    error = np.zeros(DIM, )
    if type == 1:
        target_emb = user_emb.get(target)
    else:
        target_emb = item_emb.get(target)
    update_user_item_vertex(source, target, error, 1,type)
    M = len(neg_vertices)
    if M != 0:
        for i in range(M):
            update_user_item_vertex(source, neg_vertices[i], error, 0,type)
    if source in user_social_emb and source in user_prefer_emb:
        s_v1 = user_weight_dict.get(source).get("view1")
        s_v2 = user_weight_dict.get(source).get("view2")
        s_w1 = s_v1 + error .dot (
                    user_social_emb[source] - user_prefer_emb[source])* (s_v1 ** 2) / ((s_v1 + s_v2) ** 2)
        s_w2 = s_v2 + error .dot (
                user_prefer_emb[source] - user_social_emb[source]) * (s_v2 ** 2)  / ((s_v1 + s_v2) ** 2)
        # new_vec = target_emb + g * source_emb.dot(user_user_map_matrix)
        # user_emb[target] = new_vec
        user_weight_dict[source]["view1"] = get_user_weight(s_w1)
        user_weight_dict[source]["view2"] = get_user_weight(s_w2)
        print(user_weight_dict[source]["view1"])
        print(user_social_emb[source])
        temp = user_weight_dict[source]["view1"]*user_social_emb[source]+user_weight_dict[source]["view2"]*user_prefer_emb[source]
        user_emb[source] = temp/(user_weight_dict[source]["view1"]+user_weight_dict[source]["view2"])
    else:
        user_emb[source] += error


def neg_sample_user_item(source, target, source_nei,type):
    if type == 1:
        base_list = friend_list
        neg_table = friend_neg_table
    else:
        base_list = item_list
        neg_table = item_neg_table
    # sample M negative vertices
    neg_vertices = list()
    record = 0
    while len(neg_vertices) < NEG_N:
        if record < len(base_list):
            sample_v = neg_table[random.randint(0,neg_table_size-1)]
            if sample_v not in source_nei.get(source) and sample_v not in neg_vertices:
                neg_vertices.append(sample_v)
        else:
            break
        record += 1
    update_user_item(source, target, neg_vertices,type)


def train_explicit_relation_neg(tuple_list, user_nei,type):
    t = draw_tuple(tuple_list)
    v1 = t[0]
    v2 = t[1]
    neg_sample_user_item(v1, v2, user_nei, type)


def train_user_user_relation():
    global user_user_map_matrix
    t = draw_tuple(user_friend_edges)
    v1 = t[0]
    v2 = t[1]

    bias_v1v2 = sigmoid(-user_emb.get(v1).dot(user_user_map_matrix).dot(user_emb.get(v2)))
    if (v1 in user_social_emb and v1 in user_prefer_emb) and (v2 in user_social_emb and v2 in user_prefer_emb):
        ...
        # update v1 weight
        new_v1_w1 = user_weight_dict[v1]["view1"] + explicit_lr * bias_v1v2 * (
                    user_user_map_matrix.dot(user_emb.get(v2)) * v1_w2 / ((v1_w1 + v1_w2) * (v1_w1 + v1_w2))).dot \
            (user_social_emb.get(v1) - user_prefer_emb.get(v1))
        new_v1_w2 = user_weight_dict[v1]["view2"] + explicit_lr * bias_v1v2 * (
                    user_user_map_matrix.dot(user_emb.get(v2)) * v1_w1 / ((v1_w1 + v1_w2) * (v1_w1 + v1_w2))).dot \
            (user_prefer_emb.get(v1) - user_social_emb.get(v1))
        # update v2 weight
        new_v2_w1 = user_weight_dict[v2]["view1"] + explicit_lr * bias_v1v2 * (
            user_emb.get(v1).dot(user_user_map_matrix) * v2_w2 / ((v2_w1 + v2_w2) * (v2_w1 + v2_w2))).dot(
            user_social_emb.get(v2) - user_prefer_emb.get(v2))
        new_v2_w2 = user_weight_dict[v2]["view2"] + explicit_lr * bias_v1v2 * (
            user_emb.get(v1).dot(user_user_map_matrix) * v2_w1 / ((v2_w1 + v2_w2) * (v2_w1 + v2_w2))).dot(
        ...
        # update user user map matrix
        user_user_map_matrix += explicit_lr * bias_v1v2 * user_emb[v1].reshape(DIM, 1).dot(user_emb[v2].reshape(1, DIM))
        # update robust user v1 and user v2
        ...
        new_user_v1 = user_emb.get(v1) + explicit_lr * bias_v1v2 * user_user_map_matrix.dot(user_emb[v2])
        new_user_v2 = user_emb.get(v2) + explicit_lr * bias_v1v2 * user_emb[v1].dot(user_user_map_matrix)
        user_user_map_matrix += explicit_lr * bias_v1v2 * user_emb[v1].reshape(DIM, 1).dot(user_emb[v2].reshape(1, DIM))
...
def train_user_item_relation():
    ...
    if user in user_social_emb and user in user_prefer_emb:
        ...
        new_user_w1 = user_weight_dict[user]["view1"] \
                      + explicit_lr * bias_uv * user_item_map_matrix.dot(item_emb.get(item) * v1_w2 / (
                    (v1_w1 + v1_w2) * (v1_w1 + v1_w2))).dot \
                          (user_social_emb.get(user) - user_prefer_emb.get(user))
        new_user_w2 = user_weight_dict[user]["view2"] + explicit_lr * bias_uv * (
                user_item_map_matrix.dot(item_emb.get(item)) * v1_w1 / ((v1_w1 + v1_w2) * (v1_w1 + v1_w2))).dot \
        ...
        # update vj embedding
        new_item_emb = item_emb.get(item) + explicit_lr * bias_uv * user_emb.get(user).dot(user_item_map_matrix)
        # update map_matrix
        user_item_map_matrix += explicit_lr * bias_uv * user_emb[user].reshape(DIM, 1).dot(item_emb[item].reshape(1, DIM))
        # update vj embedding
        ...
        new_user_emb = user_emb.get(user) + explicit_lr * bias_uv * user_item_map_matrix.dot(item_emb.get(item))
        new_item_emb = item_emb.get(item) + explicit_lr * bias_uv * (user_emb.get(user)).dot(user_item_map_matrix)
        user_item_map_matrix += explicit_lr * bias_uv * user_emb[user].reshape(DIM, 1).dot(item_emb[item].reshape(1, DIM))
    ...
    while iter <= ITER_ROUND:
        ...
        while view_iter < sample_T:
            if view_iter - im_last_count > 10000:
                ...
                implicit_lr = init_lr * (1 - im_current_sample_count / (1.0 * (view_iter_num + 1)))
                print("Iteration i:   " + str(im_iter) + "   ##########lr  " + str(implicit_lr))
        ...
        while link_iter < link_iter_num:
            if link_iter - ex_last_count > 10000:
                ...
                explicit_lr = init_lr * (1 - ex_current_sample_count / (1.0 * (view_iter_num + 1)))
                print("Iteration i:   " + str(ex_iter) + "   ##########lr  " + str(explicit_lr))
        ...
        if iter %4 == 0:
            ...
            pk.dump(user_user_map_matrix,
                open(out_uu_matrix + "_r" + str(reg) + "N" + str(NEG_N) + "_round" + str(iter) + ".pkl", 'wb'))
...
if __name__ == "__main__":
    ...
    pk.dump(user_user_map_matrix,open(out_uu_matrix + "_finished.pkl",'wb'))
